...
import sdkoreyunwx from "./sdkoreyunwx.js"
import gameCenter from "./game-center.js"
...
export default {
	...
	WX_SDKRYINIT(p1, p2, p3) {
        const j3 = formatJsonStr(p3);
        sdkoreyunwx.init(p1,p2,j3);
    },
	WX_SDKRYREGISTER(p1, p2) {
        const j2 = formatJsonStr(p2);
        sdkoreyunwx.register(p1,j2);
    },
	WX_SDKRYLOGGEDIN(p1, p2) {
        const j2 = formatJsonStr(p2);
        sdkoreyunwx.loggedin(p1,j2);
    },
	WX_SDKRYPAYMENT(p1, p2, p3, p4, p5) {
        const j5 = formatJsonStr(p5);
        sdkoreyunwx.payment(p1,p2,p3,p4,j5);
    },
	WX_SDKRYEVENT(p1, p2) {
        const j2 = formatJsonStr(p2);
        sdkoreyunwx.event(p1,j2);
    },
    WX_ShareImageToGameCenter(conf, callbackId) {
        const _conf = formatJsonStr(conf);
        gameCenter.shareImageToGameCenter(_conf, callbackId);
    },
    WX_ShareCanvasToGameCenter(conf, callbackId) {
        const _conf = formatJsonStr(conf);
        gameCenter.shareCanvasToGameCenter(_conf, callbackId);
    }
};
