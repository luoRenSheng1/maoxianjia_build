...
namespace WuHuan
{
    ...
    public class AssetBundleFilesAnalyzeObject
    {
        ...
        {
            var t1 = new KeyValuePair<string, object>("顶点数", mesh.vertexCount);
            var t2 = mesh.isReadable ?
                new KeyValuePair<string, object>("面数", (mesh.triangles.Length / 3f)) : new KeyValuePair<string, object>("面数", 0);
            var t3 = new KeyValuePair<string, object>("子网格数", mesh.subMeshCount);
            var t4 = new KeyValuePair<string, object>("网格压缩", MeshUtility.GetMeshCompression(mesh).ToString());
            var t5 = new KeyValuePair<string, object>("Read/Write", mesh.isReadable.ToString());
