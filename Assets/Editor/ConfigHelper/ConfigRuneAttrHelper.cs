...
{
    public class ConfigRuneAttrHelper
    {
        private static string s_ClassNameRaw = "RuneAttr";
        private static string s_ClassName = "ConfigRuneAttr";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigRuneAttr FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigRuneAttr data = new ConfigRuneAttr();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigRuneAttrUnit unit = new ConfigRuneAttrUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Chapter = ExporterUtils.GetInt(curData[1].ToString());
                    unit.RunePeak = ExporterUtils.GetInt(curData[2].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigRuneAttr Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigRuneAttr>(data, strPathOut);
