...
{
    public class ConfigDailyTaskHelper
    {
        private static string s_ClassNameRaw = "DailyTask";
        private static string s_ClassName = "ConfigDailyTask";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigDailyTask FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigDailyTask data = new ConfigDailyTask();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigDailyTaskUnit unit = new ConfigDailyTaskUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.TaskType = ExporterUtils.GetInt(curData[1].ToString());
                    unit.Type = ExporterUtils.GetInt(curData[2].ToString());
                    unit.Param = curData[3].ToString();
                    unit.Difficulty = ExporterUtils.GetInt(curData[4].ToString());
                    unit.NameNote = curData[5].ToString();
                    unit.Name = curData[6].ToString();
                    unit.Reward = curData[7].ToString();
                    unit.SystemId = ExporterUtils.GetInt(curData[8].ToString());
                    unit.Conduct = ExporterUtils.GetInt(curData[9].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigDailyTask Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigDailyTask>(data, strPathOut);
