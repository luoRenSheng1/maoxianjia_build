...
{
    public class ConfigPowerCoefficientHelper
    {
        private static string s_ClassNameRaw = "PowerCoefficient";
        private static string s_ClassName = "ConfigPowerCoefficient";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPowerCoefficient FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPowerCoefficient data = new ConfigPowerCoefficient();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigPowerCoefficientUnit unit = new ConfigPowerCoefficientUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Remarks = curData[1].ToString();
                    unit.CaseType = ExporterUtils.GetInt(curData[2].ToString());
                    unit.Type = ExporterUtils.GetInt(curData[3].ToString());
                    unit.Coefficient = ExporterUtils.GetInt(curData[4].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPowerCoefficient Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPowerCoefficient>(data, strPathOut);
