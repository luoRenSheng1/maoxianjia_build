...
{
    public class ConfigPetCodexHelper
    {
        private static string s_ClassNameRaw = "PetCodex";
        private static string s_ClassName = "ConfigPetCodex";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPetCodex FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPetCodex data = new ConfigPetCodex();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigPetCodexUnit unit = new ConfigPetCodexUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    ExporterUtils.GetArrayInt(unit.PetId, curData[1].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPetCodex Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPetCodex>(data, strPathOut);
