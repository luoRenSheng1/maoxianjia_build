...
{
    public class ConfigHeroAttrHelper
    {
        private static string s_ClassNameRaw = "HeroAttr";
        private static string s_ClassName = "ConfigHeroAttr";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigHeroAttr FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigHeroAttr data = new ConfigHeroAttr();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigHeroAttrUnit unit = new ConfigHeroAttrUnit();
                    ...
                    unit.AttrId = ExporterUtils.GetInt(curData[5].ToString());
                    unit.Value = ExporterUtils.GetInt(curData[6].ToString());
                    unit.Type = ExporterUtils.GetInt(curData[7].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigHeroAttr Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigHeroAttr>(data, strPathOut);
