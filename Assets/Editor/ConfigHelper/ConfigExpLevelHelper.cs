...
{
    public class ConfigExpLevelHelper
    {
        private static string s_ClassNameRaw = "ExpLevel";
        private static string s_ClassName = "ConfigExpLevel";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigExpLevel FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigExpLevel data = new ConfigExpLevel();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigExpLevelUnit unit = new ConfigExpLevelUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Exp = ExporterUtils.GetInt(curData[1].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigExpLevel Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigExpLevel>(data, strPathOut);
