...
{
    public class ConfigShopHelper
    {
        private static string s_ClassNameRaw = "Shop";
        private static string s_ClassName = "ConfigShop";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigShop FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigShopUnit unit = new ConfigShopUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Type = ExporterUtils.GetInt(curData[1].ToString());
                    unit.ItemId = ExporterUtils.GetInt(curData[2].ToString());
                    unit.ItemNum = ExporterUtils.GetInt(curData[3].ToString());
                    unit.Sort = ExporterUtils.GetInt(curData[4].ToString());
                    unit.Currency = ExporterUtils.GetInt(curData[5].ToString());
                    unit.Number = ExporterUtils.GetInt(curData[6].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigShop Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigShop>(data, strPathOut);
