...
{
    public class ConfigPetLotteryProHelper
    {
        private static string s_ClassNameRaw = "PetLotteryPro";
        private static string s_ClassName = "ConfigPetLotteryPro";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPetLotteryPro FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPetLotteryPro data = new ConfigPetLotteryPro();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigPetLotteryProUnit unit = new ConfigPetLotteryProUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Level = ExporterUtils.GetInt(curData[1].ToString());
                    unit.ItemType = ExporterUtils.GetInt(curData[2].ToString());
                    unit.Number = ExporterUtils.GetInt(curData[3].ToString());
                    unit.Probability = ExporterUtils.GetInt(curData[4].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPetLotteryPro Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPetLotteryPro>(data, strPathOut);
