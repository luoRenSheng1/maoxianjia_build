...
{
    public class ConfigRunePondHelper
    {
        private static string s_ClassNameRaw = "RunePond";
        private static string s_ClassName = "ConfigRunePond";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigRunePond FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigRunePond data = new ConfigRunePond();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigRunePondUnit unit = new ConfigRunePondUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Skill = ExporterUtils.GetInt(curData[1].ToString());
                    unit.Rate = ExporterUtils.GetInt(curData[2].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigRunePond Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigRunePond>(data, strPathOut);
