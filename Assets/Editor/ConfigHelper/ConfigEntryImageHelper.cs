...
{
    public class ConfigEntryImageHelper
    {
        private static string s_ClassNameRaw = "EntryImage";
        private static string s_ClassName = "ConfigEntryImage";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigEntryImage FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigEntryImage data = new ConfigEntryImage();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigEntryImageUnit unit = new ConfigEntryImageUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.NameNote = curData[1].ToString();
                    unit.Name = curData[2].ToString();
                    unit.Parts = ExporterUtils.GetInt(curData[3].ToString());
                    unit.Rate = ExporterUtils.GetInt(curData[4].ToString());
                    unit.Icon = ExporterUtils.GetInt(curData[5].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigEntryImage Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigEntryImage>(data, strPathOut);
