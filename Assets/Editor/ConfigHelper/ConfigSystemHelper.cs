...
namespace Google.Protobuf
{
    ...
    {
        private static string s_ClassNameRaw = "System";
        private static string s_ClassName = "ConfigSystem";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigSystem FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigSystem data = new ConfigSystem();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigSystemUnit unit = new ConfigSystemUnit();
                    ...
                    unit.ConditionType = ExporterUtils.GetInt(curData[4].ToString());
                    unit.Condition = curData[5].ToString();
                    unit.DescNote = curData[6].ToString();
                    unit.Desc = curData[7].ToString();
                    unit.DescParam = curData[8].ToString();
                    unit.Icon = ExporterUtils.GetInt(curData[9].ToString());
                    unit.Position = ExporterUtils.GetInt(curData[10].ToString());
                    unit.Show = ExporterUtils.GetInt(curData[11].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigSystem Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigSystem>(data, strPathOut);
