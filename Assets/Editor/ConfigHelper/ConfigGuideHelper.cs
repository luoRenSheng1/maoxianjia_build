...
{
    public class ConfigGuideHelper
    {
        private static string s_ClassNameRaw = "Guide";
        private static string s_ClassName = "ConfigGuide";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigGuide FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigGuide data = new ConfigGuide();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigGuideUnit unit = new ConfigGuideUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    ExporterUtils.GetArrayInt(unit.RelationId, curData[1].ToString());
                    unit.Tips = curData[2].ToString();
                    unit.Remarks = curData[3].ToString();
                    unit.GuideType = ExporterUtils.GetInt(curData[4].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigGuide Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigGuide>(data, strPathOut);
