using System;
using System.Collections.Generic;
using System.Data;
using System.IO;
using System.Linq;
using System.Text;
using ExcelDataReader;
using UnityEngine;
using Config;

namespace Google.Protobuf
{
    public class ConfigEventHelper
    {
        private static string s_ClassNameRaw = "Event";
        private static string s_ClassName = "ConfigEvent";

        public static void doXlsxToBin()
        {
            string strPath = ExporterUtils.GetDirPathConfig() + s_ClassNameRaw + ".xlsx";
            string strPathOut = ExporterUtils.GetDirPathConfigBin() + s_ClassName + ".bin";
            
            List<List<object>> listData = new List<List<object>>();
            List<string> listDataName = new List<string>();
            List<string> listDataType = new List<string>();
            List<string> listDataDesc = new List<string>();
            string strKeyField = "";
            FieldType keyFieldType = FieldType.INT;

            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                int nFieldNum = 14;
                
                if (listDataName.Count != nFieldNum)
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigEvent FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigEvent data = new ConfigEvent();
                
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    List<object> curData = listData[nIndex];
                    
                    if (curData == null)
                    {
                        continue;
                    }
                    
                    ConfigEventUnit unit = new ConfigEventUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.NameNote = curData[1].ToString();
                    unit.Name = curData[2].ToString();
                    unit.Type = ExporterUtils.GetInt(curData[3].ToString());
                    unit.TypeGroup = ExporterUtils.GetInt(curData[4].ToString());
                    unit.Quantity = ExporterUtils.GetInt(curData[5].ToString());
                    unit.Time = ExporterUtils.GetInt(curData[6].ToString());
                    unit.Number = ExporterUtils.GetInt(curData[7].ToString());
                    unit.Rate = ExporterUtils.GetInt(curData[8].ToString());
                    unit.Location = ExporterUtils.GetInt(curData[9].ToString());
                    unit.Range = ExporterUtils.GetInt(curData[10].ToString());
                    unit.Resource = ExporterUtils.GetInt(curData[11].ToString());
                    unit.TaskIndex = curData[12].ToString();
                    unit.Desc = curData[13].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigEvent Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigEvent>(data, strPathOut);

            }
        }
    }
}
