...
{
    public class ConfigHeroLevelHelper
    {
        private static string s_ClassNameRaw = "HeroLevel";
        private static string s_ClassName = "ConfigHeroLevel";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigHeroLevel FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigHeroLevel data = new ConfigHeroLevel();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigHeroLevelUnit unit = new ConfigHeroLevelUnit();
                    ...
                    unit.Level = ExporterUtils.GetInt(curData[4].ToString());
                    unit.Exp = ExporterUtils.GetInt(curData[5].ToString());
                    unit.Ratio = ExporterUtils.GetInt(curData[6].ToString());
                    unit.Item = curData[7].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigHeroLevel Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigHeroLevel>(data, strPathOut);
