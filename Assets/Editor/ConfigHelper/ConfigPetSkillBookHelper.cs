...
{
    public class ConfigPetSkillBookHelper
    {
        private static string s_ClassNameRaw = "PetSkillBook";
        private static string s_ClassName = "ConfigPetSkillBook";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPetSkillBook FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPetSkillBook data = new ConfigPetSkillBook();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigPetSkillBookUnit unit = new ConfigPetSkillBookUnit();
                    ...
                    unit.DocCommon = curData[7].ToString();
                    unit.Icon = ExporterUtils.GetInt(curData[8].ToString());
                    unit.Item = ExporterUtils.GetInt(curData[9].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPetSkillBook Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPetSkillBook>(data, strPathOut);
