...
{
    public class ConfigPassTaskHelper
    {
        private static string s_ClassNameRaw = "PassTask";
        private static string s_ClassName = "ConfigPassTask";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPassTask FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPassTask data = new ConfigPassTask();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigPassTaskUnit unit = new ConfigPassTaskUnit();
                    ...
                    unit.PayType = ExporterUtils.GetInt(curData[5].ToString());
                    unit.ActivityLevel = ExporterUtils.GetInt(curData[6].ToString());
                    unit.CD = ExporterUtils.GetInt(curData[7].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPassTask Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPassTask>(data, strPathOut);
