using System;
using System.Collections.Generic;
using System.Data;
using System.IO;
using System.Linq;
using System.Text;
using ExcelDataReader;
using UnityEngine;
using Config;

namespace Google.Protobuf
{
    public class ConfigTaskHelper
    {
        private static string s_ClassNameRaw = "Task";
        private static string s_ClassName = "ConfigTask";

        public static void doXlsxToBin()
        {
            string strPath = ExporterUtils.GetDirPathConfig() + s_ClassNameRaw + ".xlsx";
            string strPathOut = ExporterUtils.GetDirPathConfigBin() + s_ClassName + ".bin";
            
            List<List<object>> listData = new List<List<object>>();
            List<string> listDataName = new List<string>();
            List<string> listDataType = new List<string>();
            List<string> listDataDesc = new List<string>();
            string strKeyField = "";
            FieldType keyFieldType = FieldType.INT;

            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                int nFieldNum = 13;
                
                if (listDataName.Count != nFieldNum)
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigTask FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigTask data = new ConfigTask();
                
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    List<object> curData = listData[nIndex];
                    
                    if (curData == null)
                    {
                        continue;
                    }
                    
                    ConfigTaskUnit unit = new ConfigTaskUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.NameNote = curData[1].ToString();
                    unit.Name = curData[2].ToString();
                    unit.NameParam = curData[3].ToString();
                    unit.TaskMode = ExporterUtils.GetInt(curData[4].ToString());
                    unit.NextId = ExporterUtils.GetInt(curData[5].ToString());
                    unit.Type = ExporterUtils.GetInt(curData[6].ToString());
                    unit.TypeNameNote = curData[7].ToString();
                    unit.TypeName = curData[8].ToString();
                    unit.TypeNameParam = curData[9].ToString();
                    unit.Param1 = curData[10].ToString();
                    unit.Reward = ExporterUtils.GetInt(curData[11].ToString());
                    unit.Conduct = ExporterUtils.GetInt(curData[12].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigTask Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigTask>(data, strPathOut);

            }
        }
    }
}
