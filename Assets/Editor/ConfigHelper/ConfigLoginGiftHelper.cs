...
{
    public class ConfigLoginGiftHelper
    {
        private static string s_ClassNameRaw = "LoginGift";
        private static string s_ClassName = "ConfigLoginGift";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigLoginGift FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigLoginGift data = new ConfigLoginGift();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigLoginGiftUnit unit = new ConfigLoginGiftUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Type = ExporterUtils.GetInt(curData[1].ToString());
                    unit.Time = ExporterUtils.GetInt(curData[2].ToString());
                    unit.NameNote = curData[3].ToString();
                    unit.Name = curData[4].ToString();
                    unit.ItemId = curData[5].ToString();
                    unit.BuyNumber = ExporterUtils.GetInt(curData[6].ToString());
                    unit.PayListId = ExporterUtils.GetInt(curData[7].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigLoginGift Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigLoginGift>(data, strPathOut);
