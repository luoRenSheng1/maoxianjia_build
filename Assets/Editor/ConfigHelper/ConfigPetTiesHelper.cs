...
{
    public class ConfigPetTiesHelper
    {
        private static string s_ClassNameRaw = "PetTies";
        private static string s_ClassName = "ConfigPetTies";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPetTies FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPetTies data = new ConfigPetTies();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigPetTiesUnit unit = new ConfigPetTiesUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Name = curData[1].ToString();
                    unit.PetId = curData[2].ToString();
                    unit.AttrId = ExporterUtils.GetInt(curData[3].ToString());
                    unit.Desc = curData[4].ToString();
                    unit.Power = ExporterUtils.GetInt(curData[5].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPetTies Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPetTies>(data, strPathOut);
