using System;
using System.Collections.Generic;
using System.Data;
using System.IO;
using System.Linq;
using System.Text;
using ExcelDataReader;
using UnityEngine;
using Config;

namespace Google.Protobuf
{
    public class ConfigDailyCheckRewardHelper
    {
        private static string s_ClassNameRaw = "DailyCheckReward";
        private static string s_ClassName = "ConfigDailyCheckReward";

        public static void doXlsxToBin()
        {
            string strPath = ExporterUtils.GetDirPathConfig() + s_ClassNameRaw + ".xlsx";
            string strPathOut = ExporterUtils.GetDirPathConfigBin() + s_ClassName + ".bin";
            
            List<List<object>> listData = new List<List<object>>();
            List<string> listDataName = new List<string>();
            List<string> listDataType = new List<string>();
            List<string> listDataDesc = new List<string>();
            string strKeyField = "";
            FieldType keyFieldType = FieldType.INT;

            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                int nFieldNum = 5;
                
                if (listDataName.Count != nFieldNum)
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigDailyCheckReward FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigDailyCheckReward data = new ConfigDailyCheckReward();
                
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    List<object> curData = listData[nIndex];
                    
                    if (curData == null)
                    {
                        continue;
                    }
                    
                    ConfigDailyCheckRewardUnit unit = new ConfigDailyCheckRewardUnit();
                    
                    unit.DailyCheckId = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Count = ExporterUtils.GetInt(curData[1].ToString());
                    unit.Reward = curData[2].ToString();
                    unit.Icon = curData[3].ToString();
                    unit.Desc = curData[4].ToString();
                    
                    data.Data.Add(unit);
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigDailyCheckReward>(data, strPathOut);

            }
        }
    }
}
