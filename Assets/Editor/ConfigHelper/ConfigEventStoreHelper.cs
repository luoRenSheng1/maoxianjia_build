...
{
    public class ConfigEventStoreHelper
    {
        private static string s_ClassNameRaw = "EventStore";
        private static string s_ClassName = "ConfigEventStore";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigEventStore FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigEventStore data = new ConfigEventStore();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigEventStoreUnit unit = new ConfigEventStoreUnit();
                    ...
                    unit.Score = ExporterUtils.GetInt(curData[3].ToString());
                    unit.Limit = ExporterUtils.GetInt(curData[4].ToString());
                    unit.Sort = ExporterUtils.GetInt(curData[5].ToString());
                    unit.NameNote = curData[6].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigEventStore Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigEventStore>(data, strPathOut);
