...
{
    public class ConfigTrainingTalentHelper
    {
        private static string s_ClassNameRaw = "TrainingTalent";
        private static string s_ClassName = "ConfigTrainingTalent";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigTrainingTalent FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigTrainingTalent data = new ConfigTrainingTalent();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigTrainingTalentUnit unit = new ConfigTrainingTalentUnit();
                    ...
                    unit.HighestLevel = ExporterUtils.GetInt(curData[4].ToString());
                    unit.Layer = ExporterUtils.GetInt(curData[5].ToString());
                    unit.UnlockReq = curData[6].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigTrainingTalent Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigTrainingTalent>(data, strPathOut);
