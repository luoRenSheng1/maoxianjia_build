...
{
    public class ConfigPetAptitudeHelper
    {
        private static string s_ClassNameRaw = "PetAptitude";
        private static string s_ClassName = "ConfigPetAptitude";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPetAptitude FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPetAptitude data = new ConfigPetAptitude();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigPetAptitudeUnit unit = new ConfigPetAptitudeUnit();
                    ...
                    unit.Item = ExporterUtils.GetInt(curData[7].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPetAptitude Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPetAptitude>(data, strPathOut);
