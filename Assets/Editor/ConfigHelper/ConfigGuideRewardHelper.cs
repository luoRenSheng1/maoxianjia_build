...
{
    public class ConfigGuideRewardHelper
    {
        private static string s_ClassNameRaw = "GuideReward";
        private static string s_ClassName = "ConfigGuideReward";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigGuideReward FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigGuideReward data = new ConfigGuideReward();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigGuideRewardUnit unit = new ConfigGuideRewardUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.ChapterId = ExporterUtils.GetInt(curData[1].ToString());
                    unit.Levelid = ExporterUtils.GetInt(curData[2].ToString());
                    unit.Type = ExporterUtils.GetInt(curData[3].ToString());
                    unit.Reward = curData[4].ToString();
                    unit.PayListID = ExporterUtils.GetInt(curData[5].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigGuideReward Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigGuideReward>(data, strPathOut);
