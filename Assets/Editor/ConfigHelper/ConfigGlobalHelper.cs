...
{
    public class ConfigGlobalHelper
    {
        private static string s_ClassNameRaw = "global";
        private static string s_ClassName = "ConfigGlobal";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigGlobal FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigGlobal data = new ConfigGlobal();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigGlobalUnit unit = new ConfigGlobalUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    ExporterUtils.GetArrayInt(unit.Value, curData[1].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigGlobal Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigGlobal>(data, strPathOut);
