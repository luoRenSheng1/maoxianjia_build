...
{
    public class ConfigChapterHelper
    {
        private static string s_ClassNameRaw = "Chapter";
        private static string s_ClassName = "ConfigChapter";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigChapter FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigChapter data = new ConfigChapter();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigChapterUnit unit = new ConfigChapterUnit();
                    ...
                    unit.Scenes = ExporterUtils.GetInt(curData[4].ToString());
                    unit.MapPos = curData[5].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigChapter Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigChapter>(data, strPathOut);
