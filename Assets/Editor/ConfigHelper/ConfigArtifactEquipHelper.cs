...
{
    public class ConfigArtifactEquipHelper
    {
        private static string s_ClassNameRaw = "ArtifactEquip";
        private static string s_ClassName = "ConfigArtifactEquip";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigArtifactEquip FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigArtifactEquip data = new ConfigArtifactEquip();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigArtifactEquipUnit unit = new ConfigArtifactEquipUnit();
                    ...
                    unit.ArtifactLevel = ExporterUtils.GetInt(curData[3].ToString());
                    unit.DiamondsCost = ExporterUtils.GetInt(curData[4].ToString());
                    unit.ItemCost = curData[5].ToString();
                    unit.Attr = curData[6].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigArtifactEquip Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigArtifactEquip>(data, strPathOut);
