...
namespace Google.Protobuf
{
    ...
    {
        private static string s_ClassNameRaw = "Common";
        private static string s_ClassName = "ConfigCommon";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigCommon FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigCommon data = new ConfigCommon();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigCommonUnit unit = new ConfigCommonUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Param1 = curData[1].ToString();
                    unit.Param2 = curData[2].ToString();
                    unit.Param3 = curData[3].ToString();
                    unit.Param4 = curData[4].ToString();
                    unit.Param5 = curData[5].ToString();
                    unit.Remarks = curData[6].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigCommon Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigCommon>(data, strPathOut);
