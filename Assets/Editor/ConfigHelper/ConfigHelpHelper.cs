...
{
    public class ConfigHelpHelper
    {
        private static string s_ClassNameRaw = "Help";
        private static string s_ClassName = "ConfigHelp";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigHelp FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigHelp data = new ConfigHelp();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigHelpUnit unit = new ConfigHelpUnit();
                    ...
                    unit.ContentNote = curData[3].ToString();
                    unit.Content = curData[4].ToString();
                    unit.ContentParam = curData[5].ToString();
                    unit.Order = ExporterUtils.GetInt(curData[6].ToString());
                    unit.Icon = ExporterUtils.GetInt(curData[7].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigHelp Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigHelp>(data, strPathOut);
