...
{
    public class ConfigPetSkillBookPoolHelper
    {
        private static string s_ClassNameRaw = "PetSkillBookPool";
        private static string s_ClassName = "ConfigPetSkillBookPool";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPetSkillBookPool FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPetSkillBookPool data = new ConfigPetSkillBookPool();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigPetSkillBookPoolUnit unit = new ConfigPetSkillBookPoolUnit();
                    ...
                    unit.Interval = curData[4].ToString();
                    unit.Notes1 = curData[5].ToString();
                    unit.Notes2 = curData[6].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPetSkillBookPool Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPetSkillBookPool>(data, strPathOut);
