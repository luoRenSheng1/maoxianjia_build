...
{
    public class ConfigBoxRewardsHelper
    {
        private static string s_ClassNameRaw = "BoxRewards";
        private static string s_ClassName = "ConfigBoxRewards";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigBoxRewards FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigBoxRewards data = new ConfigBoxRewards();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigBoxRewardsUnit unit = new ConfigBoxRewardsUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Style = curData[1].ToString();
                    unit.Reward = curData[2].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigBoxRewards Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigBoxRewards>(data, strPathOut);
