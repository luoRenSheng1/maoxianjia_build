using System;
using System.Collections.Generic;
using System.Data;
using System.IO;
using System.Linq;
using System.Text;
using ExcelDataReader;
using UnityEngine;
using Config;

namespace Google.Protobuf
{
    public class ConfigPetSkillLevelHelper
    {
        private static string s_ClassNameRaw = "PetSkillLevel";
        private static string s_ClassName = "ConfigPetSkillLevel";

        public static void doXlsxToBin()
        {
            string strPath = ExporterUtils.GetDirPathConfig() + s_ClassNameRaw + ".xlsx";
            string strPathOut = ExporterUtils.GetDirPathConfigBin() + s_ClassName + ".bin";
            
            List<List<object>> listData = new List<List<object>>();
            List<string> listDataName = new List<string>();
            List<string> listDataType = new List<string>();
            List<string> listDataDesc = new List<string>();
            string strKeyField = "";
            FieldType keyFieldType = FieldType.INT;

            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                int nFieldNum = 10;
                
                if (listDataName.Count != nFieldNum)
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPetSkillLevel FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPetSkillLevel data = new ConfigPetSkillLevel();
                
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    List<object> curData = listData[nIndex];
                    
                    if (curData == null)
                    {
                        continue;
                    }
                    
                    ConfigPetSkillLevelUnit unit = new ConfigPetSkillLevelUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.SkillId = ExporterUtils.GetInt(curData[1].ToString());
                    unit.Lv = ExporterUtils.GetInt(curData[2].ToString());
                    unit.Doc = curData[3].ToString();
                    unit.SkillHurt = ExporterUtils.GetInt(curData[4].ToString());
                    unit.SkillAchieveId = curData[5].ToString();
                    unit.CoverCommonParam = curData[6].ToString();
                    unit.PetDebris = ExporterUtils.GetInt(curData[7].ToString());
                    unit.Notes = curData[8].ToString();
                    unit.Notes2 = curData[9].ToString();
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPetSkillLevel Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPetSkillLevel>(data, strPathOut);

            }
        }
    }
}
