...
{
    public class ConfigSkillBuffHelper
    {
        private static string s_ClassNameRaw = "SkillBuff";
        private static string s_ClassName = "ConfigSkillBuff";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigSkillBuff FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigSkillBuff data = new ConfigSkillBuff();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigSkillBuffUnit unit = new ConfigSkillBuffUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.BuffId = ExporterUtils.GetInt(curData[1].ToString());
                    unit.Type = ExporterUtils.GetInt(curData[2].ToString());
                    unit.Name = curData[3].ToString();
                    unit.DocNote = curData[4].ToString();
                    unit.Doc = curData[5].ToString();
                    unit.Common = curData[6].ToString();
                    unit.Time = curData[7].ToString();
                    unit.Item = ExporterUtils.GetInt(curData[8].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigSkillBuff Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigSkillBuff>(data, strPathOut);
