...
{
    public class ConfigPetLevelHelper
    {
        private static string s_ClassNameRaw = "PetLevel";
        private static string s_ClassName = "ConfigPetLevel";
        ...
        public static void doXlsxToBin()
        {
            ...
            if (ExporterUtils.ExportXlsx(strPath, ref listData, ref listDataName, ref listDataType, ref listDataDesc, ref strKeyField, ref keyFieldType))
            {
                ...
                {
                     Debug.LogError("[XlsxToBin]	doXlsxToBin Fail: ConfigPetLevel FieldNum NotMatch With Excel, Please Check.");
                     return;
                }
                
                ConfigPetLevel data = new ConfigPetLevel();
                ...
                for (int nIndex = 3; nIndex < listData.Count; ++nIndex)
                {
                    ...
                    ConfigPetLevelUnit unit = new ConfigPetLevelUnit();
                    
                    unit.Id = ExporterUtils.GetInt(curData[0].ToString());
                    unit.Quality = ExporterUtils.GetInt(curData[1].ToString());
                    unit.PetLevel = ExporterUtils.GetInt(curData[2].ToString());
                    unit.PetHurt = ExporterUtils.GetInt(curData[3].ToString());
                    unit.PetDebris = ExporterUtils.GetInt(curData[4].ToString());
                    
                    if (data.Data.ContainsKey(unit.Id))
                    {
                        Debug.LogError("[XlsxToBin] doXlsxToBin Error: ConfigPetLevel Repeat Key, Please Check. " + unit.Id);
                    }
                    
                    data.Data[unit.Id] = unit;
                }
                
                EngineBase.ConfigSerializer.Serialize<ConfigPetLevel>(data, strPathOut);
