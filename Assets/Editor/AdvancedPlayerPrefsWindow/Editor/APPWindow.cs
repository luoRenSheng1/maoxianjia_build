	...
	public class APPWindow : EditorWindow
	{
					...
					GUILayout.BeginVertical(GUILayout.Width(1));
					{
						...
						if (GUILayout.Button(new GUIContent(AddIcon, "Add a new key-value.")))
						{
							if (!string.IsNullOrEmpty(newKey))
							{
								...
								{
									case ValueType.Integer:
										UnityEngine.PlayerPrefs.SetInt(newKey, newValueInt);
										ppeList.Add(new PlayerPrefsEntry(newKey, newValueInt));
										break;
									case ValueType.Float:
										UnityEngine.PlayerPrefs.SetFloat(newKey, newValueFloat);
										ppeList.Add(new PlayerPrefsEntry(newKey, newValueFloat));
										break;
									default:
									case ValueType.String:
										UnityEngine.PlayerPrefs.SetString(newKey, newValueString);
					...
					for (int i = 0; i < filteredPpeList.Count; i++)
					{
						if (filteredPpeList[i].Value != null)
						{
							EditorGUILayout.BeginHorizontal();
							{
								...
								{
									UnityEngine.PlayerPrefs.DeleteKey(filteredPpeList[i].Key);
		...
		private class PlayerPrefsEntry
		{
			...
			public void SaveChanges()
			{
				...
				if (oldKey != Key)
				{
					UnityEngine.PlayerPrefs.DeleteKey(oldKey);
					oldKey = Key;
				}
			...
			public void RetrieveValue()
			{
				...
				if (UnityEngine.PlayerPrefs.GetString(Key, UNIQUE_STRING) != UNIQUE_STRING)
				{
					Type = ValueType.String;
					value = UnityEngine.PlayerPrefs.GetString(Key);
				}
				else if (UnityEngine.PlayerPrefs.GetInt(Key, UNIQUE_INT) != UNIQUE_INT)
				{
					Type = ValueType.Integer;
					value = UnityEngine.PlayerPrefs.GetInt(Key);
				}
				else if (UnityEngine.PlayerPrefs.GetFloat(Key, UNIQUE_FLOAT) != UNIQUE_FLOAT)
				{
