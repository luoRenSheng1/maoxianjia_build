...
public class Reporter : MonoBehaviour
{
    ...
    void drawToolBar()
    {
        GUI.skin = toolbarScrollerSkin;
        Vector2 drag = getDrag();
        if ((drag.x != 0) && (downPos != Vector2.zero) && downPos.y > size.y * 2f && downPos.y < size.y * 4f)
        {
        ...
        GUILayout.EndHorizontal();
        GUILayout.BeginHorizontal((showError) ? buttonActiveStyle : barStyle);
        ...
        if (GUILayout.Button(saveLogsContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
        {
            if (selectedLog != null)
            {
                var content = string.Format("{0}\n{1}", selectedLog.condition, selectedLog.stacktrace);
                GUIUtility.systemCopyBuffer = content;
                Debug.Log("Line logs toClipBoard");
            }
        }
#endif
        
        tempRect = GUILayoutUtility.GetLastRect();
        GUI.Label(tempRect, "  Line", lowerLeftFontStyle);
    ...
    public static System.Action<string> ActionReporterCommond;

#if PF_WEIXIN || PF_DOUYIN
    private bool isShowKeyboard = false;
    private bool isKeyboardLostForce = false;
#endif
    
    void DrawPM()
    {
        GUILayout.BeginArea(pmRect, backStyle);

        GUILayout.BeginHorizontal();

        if (pmStyle == null && GUI.skin != null && GUI.skin.textField != null)
        {
            pmStyle = new GUIStyle(GUI.skin.textField);
            pmStyle.fontSize = (int)size.y - 4;
        }

        GUI.SetNextControlName("PMCommand");
        if (pmStyle == null)
        {
            pmCommond = GUI.TextField(new Rect(0, 0, pmRect.width - size.x * 2, pmRect.height), pmCommond);
        }
        else
        {
            pmCommond = GUI.TextField(new Rect(0, 0, pmRect.width - size.x * 2, pmRect.height), pmCommond, pmStyle);
        }
        
        GUILayout.Space(pmRect.width - size.x * 2);
        
        GUI.SetNextControlName("PMCommandConfirm");
        if (GUILayout.Button("PM", barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(pmRect.height)))
        {
            if (ActionReporterCommond != null)
            {
                ActionReporterCommond(pmCommond);
            }
        }
        GUILayout.EndHorizontal();

#if PF_WEIXIN || PF_DOUYIN
        if (isKeyboardLostForce)
        {
            if (GUI.GetNameOfFocusedControl() == "PMCommand")
            {
                GUI.FocusControl("PMCommandConfirm");
            }
            isKeyboardLostForce = false;
        }
        if (GUI.GetNameOfFocusedControl() == "PMCommand" && !isShowKeyboard)
        {
            ShowKeyboard();
        }
        else if (GUI.GetNameOfFocusedControl() != "PMCommand" && isShowKeyboard)
        {
            HideKeyboard();
        }
#else
        if (Event.current.isKey && Event.current.keyCode == KeyCode.Return && GUI.GetNameOfFocusedControl() == "PMCommand")
        {
            if (ActionReporterCommond != null)
            {
                ActionReporterCommond(pmCommond);
    ...
    private void HideKeyboard()
    {
        if (isShowKeyboard)
        {
            ...
            isKeyboardLostForce = true;
        }
    }
#elif PF_DOUYIN
    public void OnInput(string value)
    {
        Debug.Log("onInput " + value);
        pmCommond = value;
    }
    ...
    private void ShowKeyboard()
    {
        if (!isShowKeyboard)
        {
            var keyBoard = StarkSDK.API.GetStarkKeyboard();
            
            keyBoard.ShowKeyboard(new StarkKeyboard.ShowKeyboardOptions()
            {
                maxLength = 999,
                multiple = false,
                defaultValue = pmCommond,
            ...
            //绑定回调
            keyBoard.onKeyboardInputEvent += OnInput;
            keyBoard.onKeyboardConfirmEvent += OnConfirm;
            keyBoard.onKeyboardCompleteEvent += OnComplete;
    ...
    public void OnGUIDraw()
    {
        ...
        logsRect.height = Screen.height * 0.75f - size.y * 5f;

        /*detailRect.x = 0f;
        detailRect.y = Screen.height - size.y * 3f;
        ...
        stackRect.x = 0f;
        stackRect.y = Screen.height * 0.75f - size.y * 5f;
        stackRect.width = Screen.width;
        stackRect.height = Screen.height * 0.25f;

        pmRect.x = 0f;
        pmRect.y = Screen.height - size.y * 5f;
        pmRect.width = Screen.width;
        pmRect.height = size.y;
        
        toolBarRect.x = 0f;
        toolBarRect.y = Screen.height - size.y * 4f;
        toolBarRect.width = Screen.width;
        toolBarRect.height = size.y * 2f;
        
        buttomRect.x = 0f;
        buttomRect.y = Screen.height - size.y * 2f;
        buttomRect.width = Screen.width;
        buttomRect.height =size.y * 2f;
    ...
    private CBaseTimer timerCheck = null;
    
    void Update()
    {
        if (timerCheck == null)
        {
            timerCheck = new CBaseTimer();
            timerCheck.Startup(5.0f);
        }

        if (timerCheck != null && timerCheck.ToNextTime())
        {
            if (Screen.lockCursor)
            {
                Debug.LogWarning("Screen.lockCursor Lock Reset To False");
